    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">Registered kitchen</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    <div class="mx-auto">
...
<br>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-header">
                      <h5 class="modal-title" id="exampleModalLabel" style=" color: #55afab;">Add kitchen Account</h5>
                    ...
                    <form method="POST" action="{{ route('kitchen.register.create') }}">
                    @csrf
...
<br>
            <table class="table table-condensed table-sm table-bordered">   
                ...
                @foreach ($kitchens as $kitchen)
                    <tr>     
                        <td style="text-align:center"><a href="#"  data-bs-toggle="modal" data-bs-target="#modifyModal{{ $kitchen->id }}"> {{ $kitchen->name }} </a></td>
                        <td style="text-align:center">{{ $kitchen->email }}</td>
                        <td style="text-align:center">{{ $kitchen->created_at }}</td>  
                        <td style="text-align:center">{{ $kitchen->status }}</td>
                        <td>
                        <!-- Button trigger activate modal -->
                  <button type="button" style="position: relative; left: 25px;" class="btn btn-success btn-sm {{ ($kitchen->status == 'Active') ? 'disabled' : '' }}"
                    data-bs-toggle="modal" data-bs-target="#activate{{ $kitchen->id }}">
                            <i class="fa-solid fa-user"></i>
                        </button>

                        <!-- Button trigger deactivate modal -->
                  <button type="button" style="position: relative; left: 40px;" class="btn btn-danger btn-sm {{ ($kitchen->status == 'Inactive') ? 'disabled' : '' }}"
                    data-bs-toggle="modal" data-bs-target="#deactivate{{ $kitchen->id }}">
                          ...
                          <!-- Modal -->
                              ...
                              <div class="modal fade" id="modalDialogScrollable{{ $kitchen->id }}" tabindex="-1">
                                <div class="modal-dialog modal-dialog-scrollable">
                                  <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                    <h6><strong>Name:</strong> {{ $kitchen->name }}</h6><br>
                                    <h6><strong>Username:</strong> {{ $kitchen->email }}</h6><br>
                                    <h6><strong>Registered at:</strong> {{ $kitchen->created_at }}</h6><br>
                                    </div>
                                    <div class="modal-footer">
...
<div class="modal fade" id="deactivate{{ $kitchen->id }}" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="{{ route('kitchen.deactivate', $kitchen->id) }}">
                @csrf
                @method('PATCH')
                <div class="modal-header">
 ...
 <!-- activate modal -->
 <div class="modal fade" id="activate{{ $kitchen->id }}" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="{{ route('kitchen.activate', $kitchen->id) }}">
                @csrf
                @method('PATCH')
                <div class="modal-header">
...
<div class="modal fade" id="modifyModal{{$kitchen->id}}" tabindex="-1" aria-labelledby="modifyModalLabel"
    aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modifyModalLabel" style=" color: #55afab;">Update kitchen Account</h5>
                <button type="button" class="close" style="color: #E21818; font-size: 25px;" data-bs-dismiss="modal">&times;</button>
            </div>
        <div class="modal-body">
            <div class="flex items-center justify-center bg-white-100">
            <hr class="bg-[#55AFAB] border-0">
            <form method="POST" class="updateaccount" action="{{ route('admin.register.update', $kitchen->id) }}">
                @csrf
                <div class="w-[450px] pb-4">
                <div class="py-2" >
                    <span class="mb-2 text-sm font-semibold">Name <span
                    ...
                    <input type="hidden" name="accounttype" value="kitchen" /> 
                    <input type="text" name="name" value="{{$kitchen->name}}" required autofocus id="name"
                    class="w-full p-1 border border-gray-300 rounded-md placeholder:font:light placeholder:text-gray-500 placeholder:text-sm py-1"
                    placeholder="Enter your name">
                    <x-input-error :messages="$errors->get('name')" class="mt-2" />
                </div>
                <div class="py-2" >
                    <span class="mb-2 text-sm font-semibold">Username
                        <span class="text-red-500 text-sm ">*</span> </span>
                    <input type="text" name="email" value="{{$kitchen->email}}" required id="email"
                        class="w-full p-1 border border-gray-300 rounded-md placeholder:font:light placeholder:text-gray-500 placeholder:text-sm py-1"
                        placeholder="Enter your Username">
                        <x-input-error :messages="$errors->get('email')" class="mt-2" />
                </div>
