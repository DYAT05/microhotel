...
<body>
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      <h1>Overall Restaurant Sales</h1>
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Home</a></li>
          <li class="breadcrumb-item active">Overall Restaurant Sales</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
      ...
      <div class="row">
        ...
        <div class="col-lg-12">
          <div class="row">
            ...
            <form id="filter-form" action="{{ route('admin.overallsales.preview') }}" method="get" class="mb-[50px]">
                <div class="flex item-center">
 {{-- DYAT --}}
                  {{-- <div class="mr-10">
                    <div>Date From</div>
                      <input type="date" name="dateFrom" id="dateFrom" value="{{ $dateFrom }}" class="block w-[170px] mt-1 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md">
                  </div>
                  <div class="mr-10">
                    <div>Date To</div>
                      <input type="date" name="dateTo" id="dateTo" value="{{ $dateTo }}" class="block w-[170px] mt-1 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md">
                  </div> --}}
                  <div class="mr-10">
                    <div>Date From</div>
                    <input type="date" name="dateFrom" id="dateFrom" value="{{ $dateFrom }}" class="block w-[170px] mt-1 py-2 border-gray-300 rounded-md">

                    <div class="mt-2">Time From</div>
                    <input type="time" name="timeFrom" id="timeFrom" value="{{ $timeFrom ?? '' }}" class="block w-[170px] mt-1 py-2 border-gray-300 rounded-md">
                  </div>

                  <div class="mr-10">
                    <div>Date To</div>
                    <input type="date" name="dateTo" id="dateTo" value="{{ $dateTo }}" class="block w-[170px] mt-1 py-2 border-gray-300 rounded-md">

                    <div class="mt-2">Time To</div>
                    <input type="time" name="timeTo" id="timeTo" value="{{ $timeTo ?? '' }}" class="block w-[170px] mt-1 py-2 border-gray-300 rounded-md">
                  </div>
{{-- DYAT --}}
                  <div class="mr-10">
                  ...
                  <div class="mr-10">
                    <div class="opacity-1"></div>
                      <button type="button" id="resetBtn" style="position: relative; top: 20px;" class="bg-[#259F6C] w-[120px] mt-1 py-2 text-white rounded-md">Reset</button>
                  </div>
                </div>
            </form>
            ...
            <table id="datatable" class="table table-condensed table-sm table-bordered">   
                <thead class="bg-[#51bdb8] text-white">   
                    <tr class="text-center ">   
                        <th scope="col" class="text-center px-1 py-3">Ordered Date</th>
                        <th scope="col" class="text-center px-1 py-3">Total Amount</th>
                        <th scope="col" class="text-center px-1 py-3">Cashier on Duty</th>
                    </tr>   
                </thead>   
                </tbody> 
                @foreach ($reports as $index => $report)
                <tr class="text-center py-5" >
                    <td scope="col">{{ $report->created_at }}</td>
                    <td scope="col">{{ number_format($report->total_amount, 2) }}</td>
                    <td scope="col">{{ $report->cashiername }}</td> 
                </tr>
                @endforeach  
            </table>
...
<script>
  $(document).ready(function() {
      $('#resetBtn').click(function() {
        window.location.href = "{{ route('admin.overallsales') }}";
      });
  });
  // $(document).ready(function() {
  //   $('#resetBtn').click(function() {
  ...
  //     // Optional: if you want to also reload the page
  //     // location.reload();
  //   });
  // });
