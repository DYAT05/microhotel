            ...
            <table class="table table-condensed table-sm table-bordered">   
                ...
                @foreach ($frontdesks as $frontdesk)
                    <tr>     
                        ...
                        <td style="text-align:center"><a href="#"  data-bs-toggle="modal" data-bs-target="#modifyModal{{ $frontdesk->id }}"> {{ $frontdesk->name }} </a></td>
                  ...
                  <button type="button" style="position: relative; left: 25px;" class="btn btn-success btn-sm {{ ($frontdesk->Acc_Stat == 'Active') ? 'disabled' : '' }}"
                    data-bs-toggle="modal" data-bs-target="#active{{ $frontdesk->id }}">
                            <i class="fa-solid fa-user"></i>
                        </button>

                        <!-- Button trigger deactivate modal -->
                  <button type="button" style="position: relative; left: 40px;" class="btn btn-danger btn-sm {{ ($frontdesk->Acc_Stat == 'Inactive') ? 'disabled' : '' }}"
                    data-bs-toggle="modal" data-bs-target="#inactive{{ $frontdesk->id }}">
                              ...
                              <div class="modal fade" id="modalDialogScrollable{{ $frontdesk->id }}" tabindex="-1">
                                <div class="modal-dialog modal-dialog-scrollable">
                                  <div class="modal-content">
                                    ...
                                    <h6><strong>Username:</strong> {{ $frontdesk->email }}</h6><br>
...
<div class="modal fade" id="inactive{{ $frontdesk->id }}" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="{{ route('frontdesk.inactive', $frontdesk->id) }}">
                @csrf
                @method('PATCH')
                <div class="modal-header">
 ...
 <!-- activate modal -->
 <div class="modal fade" id="active{{ $frontdesk->id }}" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="{{ route('frontdesk.active', $frontdesk->id) }}">
                @csrf
                @method('PATCH')
                <div class="modal-header">
...
<div class="modal fade" id="modifyModal{{$frontdesk->id}}" tabindex="-1" aria-labelledby="modifyModalLabel"
    aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modifyModalLabel" style=" color: #55afab;">Update Frontdesk Account</h5>
                <button type="button" class="close" style="color: #E21818; font-size: 25px;" data-bs-dismiss="modal">&times;</button>
            </div>
        <div class="modal-body">
            <div class="flex items-center justify-center bg-white-100">
            <hr class="bg-[#55AFAB] border-0">
            <form method="POST" class="updateaccount" action="{{ route('admin.register.update', $frontdesk->id) }}">
                @csrf
                <div class="w-[450px] pb-4">
                <div class="py-2" >
                    <span class="mb-2 text-sm font-semibold">Name <span
                    ...
                    <input type="hidden" name="accounttype" value="frontdesk" /> 
                    <input type="text" name="name" value="{{$frontdesk->name}}" required autofocus id="name"
                    class="w-full p-1 border border-gray-300 rounded-md placeholder:font:light placeholder:text-gray-500 placeholder:text-sm py-1"
                    placeholder="Enter your name">
                    <x-input-error :messages="$errors->get('name')" class="mt-2" />
                </div>
                <div class="py-2" >
                    <span class="mb-2 text-sm font-semibold">Username
                        <span class="text-red-500 text-sm ">*</span> </span>
                    <input type="text" name="email" value="{{$frontdesk->email}}" required id="email"
                        class="w-full p-1 border border-gray-300 rounded-md placeholder:font:light placeholder:text-gray-500 placeholder:text-sm py-1"
                        placeholder="Enter your Username">
                        <x-input-error :messages="$errors->get('email')" class="mt-2" />
                </div>
