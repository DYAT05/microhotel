    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">Registered Stock Controllers</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    <div class="mx-auto">
...
<br>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-header">
                      <h5 class="modal-title" id="exampleModalLabel" style=" color: #55afab;">Add Stock Controller Account</h5>
                    ...
                    <form method="POST" action="{{ route('stockcontroller.register.create') }}">
                    @csrf
                        ...
                        <x-input-error :messages="$errors->get('name')" class="mt-2" />
                          ...
                          <div class="py-2" >
                        <span class="mb-2 text-sm font-semibold">Username
                            <span class="text-red-500 text-sm ">*</span> </span>
                        <input type="text" name="email" :value="old('email')" required id="email"
                            class="w-full p-1 border border-gray-300 rounded-md placeholder:font:light placeholder:text-gray-500 placeholder:text-sm py-1"
...
<br>
            <table class="table table-condensed table-sm table-bordered">   
                ...
                <tbody>   
                @foreach ($stockcontrollers as $stockcontroller)
                    <tr>     
                        <td style="text-align:center"><a href="#"  data-bs-toggle="modal" data-bs-target="#modifyModal{{ $stockcontroller->id }}"> {{ $stockcontroller->name }} </a></td>
                        <td style="text-align:center">{{ $stockcontroller->email }}</td>
                        <td style="text-align:center">{{ $stockcontroller->created_at }}</td>  
                        <td style="text-align:center">{{ $stockcontroller->status }}</td>
                        <td>
                        <!-- Button trigger activate modal -->
                  <button type="button" style="position: relative; left: 25px;" class="btn btn-success btn-sm {{ ($stockcontroller->status == 'Active') ? 'disabled' : '' }}"
                    data-bs-toggle="modal" data-bs-target="#activate{{ $stockcontroller->id }}">
                            <i class="fa-solid fa-user"></i>
                        </button>

                        <!-- Button trigger deactivate modal -->
                  <button type="button" style="position: relative; left: 40px;" class="btn btn-danger btn-sm {{ ($stockcontroller->status == 'Inactive') ? 'disabled' : '' }}"
                    data-bs-toggle="modal" data-bs-target="#deactivate{{ $stockcontroller->id }}">
                          ...
                          <!-- Modal -->
                              ...
                              <div class="modal fade" id="modalDialogScrollable{{ $stockcontroller->id }}" tabindex="-1">
                                <div class="modal-dialog modal-dialog-scrollable">
                                  <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                    <h6><strong>Name:</strong> {{ $stockcontroller->name }}</h6><br>
                                    <h6><strong>Username:</strong> {{ $stockcontroller->email }}</h6><br>
                                    <h6><strong>Registered at:</strong> {{ $stockcontroller->created_at }}</h6><br>
                                    </div>
                                    <div class="modal-footer">
...
<div class="modal fade" id="deactivate{{ $stockcontroller->id }}" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="{{ route('stockcontroller.deactivate', $stockcontroller->id) }}">
                @csrf
                @method('PATCH')
                <div class="modal-header">
 ...
 <!-- activate modal -->
 <div class="modal fade" id="activate{{ $stockcontroller->id }}" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="{{ route('stockcontroller.activate', $stockcontroller->id) }}">
                @csrf
                @method('PATCH')
                <div class="modal-header">
...
<div class="modal fade" id="modifyModal{{$stockcontroller->id}}" tabindex="-1" aria-labelledby="modifyModalLabel"
    aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modifyModalLabel" style=" color: #55afab;">Update Stock Controller Account</h5>
                <button type="button" class="close" style="color: #E21818; font-size: 25px;" data-bs-dismiss="modal">&times;</button>
            </div>
        <div class="modal-body">
            <div class="flex items-center justify-center bg-white-100">
            <hr class="bg-[#55AFAB] border-0">
            <form method="POST" class="updateaccount" action="{{ route('admin.register.update', $stockcontroller->id) }}">
                @csrf
                <div class="w-[450px] pb-4">
                <div class="py-2" >
                    <span class="mb-2 text-sm font-semibold">Name <span
                    ...
                    <input type="hidden" name="accounttype" value="stockcontroller" /> 
                    <input type="text" name="name" value="{{$stockcontroller->name}}" required autofocus id="name"
                    class="w-full p-1 border border-gray-300 rounded-md placeholder:font:light placeholder:text-gray-500 placeholder:text-sm py-1"
                    placeholder="Enter your name">
                    <x-input-error :messages="$errors->get('name')" class="mt-2" />
                </div>
                <div class="py-2" >
                    <span class="mb-2 text-sm font-semibold">Username
                        <span class="text-red-500 text-sm ">*</span> </span>
                    <input type="text" name="email" value="{{$stockcontroller->email}}" required id="email"
                        class="w-full p-1 border border-gray-300 rounded-md placeholder:font:light placeholder:text-gray-500 placeholder:text-sm py-1"
                        placeholder="Enter your Username">
                        <x-input-error :messages="$errors->get('email')" class="mt-2" />
                </div>
            ...
            <div class="modal-footer">
            <button type="button" class="btn btn-secondary" style="width:85px;" data-bs-dismiss="modal">Close</button>
            <button type="submit" class="btn btn-success" style="width:85px;">Update</button>
            </form>
            </div>
    </div>
</div>
  ...
  <script>
    $('.updateaccount').submit(function() {
        var re = confirm("Are you sure you want to update this account?");
        return re;
    });
  </script>
</body>
