    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">New Registered kitchen</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    <div class="mx-auto">
            ...
            <table class="table table-condensed table-sm table-bordered">   
                ...
                @foreach ($kitchens as $kitchen)
                    <tr>     
                        ...
                        <td style="text-align:center">{{ $kitchen->status }}d</td>
                        <td>
                        <!-- Button trigger activate modal -->
                  <button type="button" style="position: relative; left: 20px;" class="btn btn-success btn-sm {{ ($kitchen->status == 'Activate') ? 'disabled' : '' }}"
                    data-bs-toggle="modal" data-bs-target="#activate{{ $kitchen->id }}">
                            <i class="fa-solid fa-user"></i>
                        </button>

                        <!-- Button trigger deactivate modal -->
                  <button type="button" style="position: relative; left: 25px;" class="btn btn-danger btn-sm {{ ($kitchen->status == 'Deactivate') ? 'disabled' : '' }}"
                              ...
                              <div class="modal fade" id="modalDialogScrollable{{ $kitchen->id }}" tabindex="-1">
                                <div class="modal-dialog modal-dialog-scrollable">
                                  <div class="modal-content">
                                    ...
                                    <h6><strong>Email:</strong> {{ $kitchen->email }}</h6><br>
...
<div class="modal fade" id="modifyModal{{$kitchen->id}}" tabindex="-1" aria-labelledby="modifyModalLabel"
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-body">
            <div class="flex items-center justify-center bg-white-100">
            <hr class="bg-[#55AFAB] border-0">
            <form method="POST" action="{{ route('admin.register.update', $kitchen->id) }}">
                @csrf
                <div class="w-[450px] pb-4">
                <div class="py-2" >
                    <span class="mb-2 text-sm font-semibold">Name <span
                ...
                <div class="py-2" >
                    <span class="mb-2 text-sm font-semibold">Email
                        <span class="text-red-500 text-sm ">*</span> </span>
                    <input type="email" name="email" value="{{$kitchen->email}}" required id="email"
                        class="w-full p-1 border border-gray-300 rounded-md placeholder:font:light placeholder:text-gray-500 placeholder:text-sm py-1"
                        placeholder="Enter your email">
                        <x-input-error :messages="$errors->get('email')" class="mt-2" />
                </div>
