...
<body>
  ...
  <aside id="sidebar" class="sidebar">
    ...
    <ul class="sidebar-nav" id="sidebar-nav">
      ...
      <li class="nav-item">
        <a class="nav-link nav-link-icon " href="{{ route('admin.roomtype.index') }}">
          <i class="fa-solid fa-bed icon-nav"></i>
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      <h1>FAQ Maintenance</h1>
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Home</a></li>
          <li class="breadcrumb-item active">FAQ Maintenance</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" style="color: #55afab">Add New FAQ</h4>
      ...
      @foreach ($faqs as $faq)
          <tr>     
              ...
              <!--View Button-->	
                ...
                <button type="button"  class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#editFAQ{{ $faq->id }}">
                    <i class="fa-solid fa-pencil"></i>
                </button>
                <button type="button"  class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#deletefaq{{ $faq->id }}">
...
<div class="modal fade" id="editFAQ{{$faq->id}}">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" style="color: #55afab">Edit FAQ</h4>
                <button type="button"  style="color: #E21818; font-size: 25px;" data-bs-dismiss="modal">&times;</button>
            </div>
            <div class="modal-body">
                <form action="{{ route('admin.faq.update',  $faq->id) }}" class="editfaqform" method="POST" enctype="multipart/form-data">
                    @csrf

                    <div class="form-group mb-2 font-semibold">
                        <label>FAQ Question</label>
                        <input class="form-control" type="text" name="question" placeholder="Enter Question" value="{{$faq->question}}">
                        <x-input-error :messages="$errors->get('question')"/>
                    </div>
                    <div class="form-group mb-2 font-semibold">
                        <label>Answer</label>
                        <input class="form-control" type="text" name="answer" placeholder="Enter Answer" value="{{$faq->answer}}">
                        <x-input-error :messages="$errors->get('answer')"/>
                    </div>
                    <button type="submit" style="position: relative; left: 400px;" class="btn btn-primary">Save</button>
                    <button type="reset" data-bs-dismiss="modal" style="position: relative; left: 260px; background-color: #7c7c7c; border: none;" class="btn btn-primary">Cancel</button>
...
<div class="modal fade" id="deletefaq{{ $faq->id }}" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" class="deletefaqform" action="{{ route('admin.deleteFaq', $faq->id) }}">
                @csrf
                @method('PATCH')
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel"  style="color: #55afab;">Delete FAQ</h5>
  ...
  <script src="{{ asset('template/assets/js/main.js') }}"></script>
    ...
    <script>
        ...
        $('.editfaqform').submit(function() {
            var ra = confirm("Are you sure you want to edit this FAQ?");
            return ra;
        });
